...
#### 跨文件一致性稽核（必做，輸出 ✅ 一致 / ⚠️ 待修正）

| 檢查項目 | 確認內容 |
|---------|---------|
| **欄位名稱** | PRD TypeScript 介面的欄位名稱，與 Mockups Spec 資料契約中的名稱一致？ |
| **情境名稱** | PRD 的場景名稱（如 `empty`、`normal`、`parseFailed`），與 Mockups Must-pass Stories 一致？ |
| **狀態矩陣** | PRD 每個區塊，在 Mockups 狀態矩陣中都有對應的 loading/empty/error/success 覆蓋？ |
| **商業規則** | PRD 的欄位限制（字數、數量上限），在 Mockups UI 規格的驗證說明中也有記載？ |

任何 ⚠️ 待修正項目，必須在宣告可交棒前修正完畢。
